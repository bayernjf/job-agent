...
> 文档版本：v0.2
> 日期：2026-09-10
> 状态：**#1–#8 已于 2026-09-10 拍板（见《待拍板决策清单》）；#4 当日修订为"海内外同步"**。v0.2 据决策结果与《市场调研-AI求职赛道》回灌，v0.1 的 `【待决策 #n】` 标记统一转为"已决策"。
> 关联文档：
...
- **非目标（本期明确不追求）**：不做双边交易闭环、不追求岗位 / 简历规模、不做支付；**不做自动投递、ATS 表单自动填充、内推自动化**（这些是效率工具主场，JobAgent 定位为可被其携带/互操作的"可信画像消费端"，依据《市场调研-AI求职赛道》第 6 章）。
