...
| # | 任务 | 完成判据 | 依赖 |
| --- | --- | --- | --- |
| ✅ T28 | **把 L0 早返回收回 PRD 原意**（P0）：批次 6 把它做成了每次分析的无条件主路径，于是同一 profileId 先后给出两份结论（实测 L0-only `mixed_signals/0.6/1 题` vs L0+L1 `likely_authentic/0.77/2 题`），违背 AGENTS 第 5 条不可变快照原则；且 `by-subject` 现接受 `partial`，扩展可能抓走临时态 | **2026-09-25 晚已落地**：worker 改成"等 L1 到齐后**一次性**发布"，只有 L1 真失败/预算耗尽（含 `collectStagedL1` 意外抛错）才落一份 `partial` **终态**（`missing:['l1_failed']` + `analysisLayers:['L0']`），进度只走 `jobs.stage`；`profiles.updateSnapshot` 与 `ProfileSnapshotPatch` 整体删除（无使用者不留死 API）；用例断言 `profiles.insert` **恰好一次** | — |
| ✅ T29 | **给批次 6 的三个新前端表面补浏览器级验证**（P1）：`/my`（登录态 + 空态）、自选岗位表单（填→提交→简历预览）、partial 提示条与 `?unavailable=1` 分流；另修三件事——简历岛不再让零技能画像彻底没有出路（与 T15 同族），`apps/api` 显式 testTimeout 或给 `interviews`/`resume` 两条撞线用例留余量，报告 E2E 的 **Astro dev SSR 冷路由**要在正式用例前预热（2026-09-25 CI 实测：`gated-content.spec.ts:55` 在 `ef622c0` 的 push run 上 `page.goto` 30s 超时两次，同一 SHA 的 pull_request run 却全绿；同 run 里首访该 Gitee 路由的用例耗时 9.1s，其余报告页用例 150–500ms ⇒ 冷编译/资源争用，非确定性缺陷，2026-09-26 在 `2a2590d` 上两个 run 均绿） | **2026-09-26 已落地**：新 `e2e/batch6-surfaces.spec.ts` 7 用例真覆盖三表面（`/my` 三态、partial+分流、自选岗位表单 mock `resumes/build` 全链路）；简历岛去掉 `skillTags.length>0` 门控；`apps/api/vitest.config.ts` 显式 15s timeout；`e2e/preload.ts` 冷路由预热挂到字母序首 spec 与 gated-content。**实测修掉两个真问题**：fixture 手写无版本前缀信号 code → `composeSignalLabel` 增 `fallbackLabel` 未知 code 退快照英文原句（en 不变原则）；自选表单冷路由 hydration 竞态致按钮 disabled → 用例 fill 后 4s 等启用、未启用则重填兜底再点。全量 `pnpm e2e` **77/77**（首跑 75/77 → 修复后全绿，report-render + batch6 18/18 单跑验证） | — |
| ✅ T30 | **岗位池口径对齐**（P2）：`GET /job-postings/stats` 已如实报 `active:0`（7 天新鲜窗口），CLI `jobs stats` 仍报 `active=2303`（状态列），同一件事两个表面讲两个故事 | **2026-09-26 已落地**：`runStats` 增列"新鲜窗口内 N 条"（`freshWithin7d`，与 API 同用 `countActiveFresh(cutoff)` + `JOB_STALE_DAYS` 默认 7）与 `cutoff=<ISO>` 语义；输出 `<source>\tactive=N\tinactive=N\tfreshWithin7d=N` + total 行。cutoff 时钟改用 `deps.now` 注入（测试 harness 固定时钟下首跑断言失败暴露，裸 `Date.now()` 不可测）。`jobs.test.ts` 该用例补 `freshWithin7d=1`/`cutoff=` 断言，7/7 绿 | T24 |
...
| ✅ T33 | **内核散文里剩下的那些句子还翻不动，因为画像没存可复算的事实**（P1，T23 拆出来的另一半）：①面试准备包 `interview-kit.ts:128` 原样打 `signal.label`/`signal.detail`（`AuthenticitySignal` 只有 code + 散文，百分比/比率在文字里，渲染侧拿不到数）；②同一文件打 `interviewQuestions[].question`/`intent`（连 code 都没有）；③报告页 `report/[profileId].astro:507/534` 打 `cadenceSummary` 与带外部贡献片段的 `prSummary`——`prSummaryFactsFromProfile` 今天**只能**取 `totalPullRequests`/`mergedPullRequests`，因为"外部 merged PR 数"没进快照，而 `externalMergedContributions` 是去重后 ≤5 个仓库名，拿它当 PR 数就是造假 | **2026-09-26 已落地**：`shared` 增 `SignalFactsSchema` + 信号 `facts?`、`InterviewQuestionKindSchema` 枚举 + 题目 `kind?`/`facts?`、composer `composeSignalLabel`（含 `fallbackLabel` 未知 code 退快照英文）/`composeSignalDetail`/`composeInterviewIntent`/`composeCadenceSummary`，`prSummaryFactsFromProfile` 增读 `externalMergedPullRequests`；`analyzer-core` 全部 11 条信号补 `facts`、5 类面试题补 `kind`+`facts`、`activity.metrics` 补 `externalMergedPullRequests`（`!repoOwnerIsSelf && MERGED` 计数）→ 规则 **0.6→0.7**；报告页 `[profileId].astro` 与 `interview-kit.ts` 三处只认 code+facts 现拼（facts 缺失退快照英文原句）。守卫：`expectFactsForAllSignals` 断言每条信号 facts 非空 + shared composer 6 用例（含未知 code 回退）+ profile 确定性用例钉 metrics。验收口径（中文面 grep 不到内核散文）由 E2E/render 面既有用例守护 | T23 |
...
**T01–T03 与 T04–T08 并行开**（隐私与表达力无耦合，**均已于 2026-09-25 落地**）→ **T11**（一条小修复，立刻让简历能投出去）→ **批次 6（T24–T27 + T17 提前）**：09-25 评审判定这四条是"核心完全可用"的真阻断，排在批次 2–4 之前，**已于 2026-09-25 全部落地** → T09/T10 → T12–T16 → T18–T20；**T21/T22 越早越好**，否则批次 4 做完仍是"对着空岗位池挑"。
