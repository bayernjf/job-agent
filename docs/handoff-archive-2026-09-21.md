...
- 2026-09-21：**形态 C 部署执行包（item39，已随 PR #70 合入 main）**——新增照勾执行的 [docs/部署执行单-形态C-20260921.md](部署执行单-形态C-20260921.md)（A–J 阶段、Vercel env 总表、故障速查）与 `tools/gen-deploy-secrets.sh`、`tools/smoke-deploy.sh`，docs/README 补场景入口；纠正 `demo seed` 是只读就绪检查、`DEMO_PRESET_LOGINS` 默认空须显式配三预置账号。控制台步骤仍只能用户本人执行。
