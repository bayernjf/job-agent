...
| #12 Gitee / 多证据源接入 | 契约层已预留 `EvidenceSource` 抽象；先用 GitHub 跑通 L0/L1 内核，避免最早期两套 API/合规翻倍 | ✅ 触发条件 2026-09-13 已满足（GitHub L0/L1 三轮校准 26 账号 0 误报/0 漏报、决策 #4"海内外同步"已拍板），待拍板重启为第一个并行证据源；开工前先实测 Gitee 公开 API 的行为数据丰富度（见 handoff item 12 第二档） | [待拍板决策清单 #12/#4](待拍板决策清单-20260910.md) |
