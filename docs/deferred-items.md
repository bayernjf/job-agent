...
| B/C 完整后台（招聘方工作台 / 应聘者面板） | 先用落地页轻量入口验证需求，过早建双边后台会摊薄资源、拖慢验证。**2026-09-25 实查：本行触发条件已满足且工作台已部分落地**——去风险实验已完成（26 账号三轮双向校准，handoff item5/item26），P2 系统层已落地（职位聚合 item10、匹配 item12），招聘侧工作台/投递追踪/面试计划已落（item21/item45）。**本行剩余未落地的只有"角色与组织"这一层**，已由 #17 单独立项，故本行不再作为整体挂起项 | 已触发（见左）；后续按 #17 与设计文档推进 | [PRD 阶段规划](PRD.md)、[讨论记录 01](讨论记录-01-切入口与MVP收敛-20260910.md)、[design-recruiter-roles-20260925](design-recruiter-roles-20260925.md) |
| **B 端组织 / 团队 / 席位与计费（#17 的第二期）**：`teams` + `memberships`、行级 `team_id` 归属、多面试官各自提交评价、席位配额与付费主体 | 2026-09-25 选定 #17 走**分期**：第一期只做"角色声明 + 访问闸"（零新表，**#17 已拍板（第一期）、代码未开工**，见 [design-recruiter-roles-20260925](design-recruiter-roles-20260925.md) 与 handoff item59）。组织模型在一个外部招聘方真实用户都没有时必然是猜的；一旦定错要连带改 012 `interviews` 的 `created_by_account_id` 归属语义，成本随已落地数据单调上升 | **二者先到者触发**：① 出现 ≥2 个真实外部账号声明招聘方（需要共享候选人池 / 行级可见性）② #9 商业模式拍板要上 B 端计费（需要付费主体与席位）。触发前不得预建 `teams` 表 | [design-recruiter-roles-20260925 §2.3 / §3 / §5](design-recruiter-roles-20260925.md)、[待拍板决策清单 #9](待拍板决策清单-20260910.md) |
...
| JSON API 字段级授权裁剪（未登录调用 `/profiles/:id` 时连面试题文本等也裁掉） | 报告页人机界面已墙、`exportable` 投影本就无证据 URL/面试题，扩展一键填充依赖其公开。**口径纠正（2026-09-25 评审实查，原文"当前未登录 API 面基本拿不到证据 URL"不成立）**：`GET /profiles/:id` 返回整份 snapshot（含 `interviewQuestions`），`GET /profiles/:id/job-recommendations` 经 `apps/api/src/match-explain.ts:81` 匿名返回证据 `url`+`claim`——**页面那道墙只约束页面，不是访问控制**，对外文案不得称"登录才能看到证据"；过早裁剪仍会破坏扩展与分享链路，故继续缓做 | 对外公开分享后出现 API 被批量抓取/搬运画像内容的滥用，或招聘方滥用投诉时 | [API.md §1.2](API.md)、[design-auth-gating-20260919](design-auth-gating-20260919.md) |
...
| **招聘方代传简历 → 建画像 / 进雇佣决策**（B 端简历导入，方案 3） | 现状只处理「公开行为数据 + 本人 OAuth 授权」（PRD 数据来源合法性硬边界）。代传简历引入**第三方直接 PII**（姓名/电话/邮箱/雇主），同意只能由数据主体给、招聘方给不出；招聘/筛选在欧盟属 AI **高风险**用途（透明度/人类监督/日志/偏差评估），《个保法》对自动化决策另有说明与拒绝权；隐私政策权威源与 CWS 权限披露都得重写；境内做成"帮企业筛人"还可能触及人力资源服务经营资质 | **三者齐备前不得开工**：①法务书面意见（含 GDPR/个保法/AI Act 当期生效状态）②隐私政策与扩展披露重写③境内资质确认。期间只做下面的"本人上传"版 | PRD「数据来源合法性 / 自动化决策与画像风险」、待拍板决策清单 #1/#6（选项 B 原话：触及画像/自动化决策合规红线）、#14 |
| **简历 ↔ 画像「对账」：本人上传自己简历，输出 证据支持 / 矛盾 / 无证据 三态**（方案 1） | 数据主体即用户本人，合规面与现状同级；且它才是"护城河=鉴别真实性"最锋利的落地——DINQ 与开源 Hiring Agent 只读不证伪。代价：PDF 文本层抽取质量（中英混排/分栏/扫描件）、新增输入面与原文处置策略 | 先用 5 份真实中英混排简历做**文本抽取 spike**，抽净率达标再进 PRD 排期；开工时"原文不留存、只留结构化声明"必须**代码级强制**（不靠约定） | docs/产品构想 §2.3（护城河＝鉴别+深析）、design-targeted-resume §10（no-fabrication / 不入库）、handoff item58 |
| **「未经本人授权」标注覆盖其余公开出口**（`GET /profiles/:id` JSON、`/exportable`、扩展面板、分享/SEO 摘要） | 2026-09-25 已落报告页提示条与企业工作台候选人卡片两处（handoff item58），但 JSON API 等机器可读出口仍输出真实性结论而无标注，第三方抓取时最易断章取义 | 出现画像结论被第三方当事实引用/搬运的实例，或对外提供 API 集成、或落地页/搜索摘要开始展示真实性结论时 | handoff item58、design-auth-gating-20260919.md（可见性矩阵）、待拍板决策清单 #6 落地状态 |
| **招聘方查看候选人画像 → 回告本人（"谁看了我"）**（#17 第二期的合规面） | 决策 #1-A 定的是"C 授权为主脊 + B 端公开轻预览"，**从未承诺**告知候选人"谁看过我"。且实现前提是**不存在**的：今天招聘方没有身份（`accounts` 无角色列）、`/candidates` 匿名可遍历，所以"谁"无法回答；一旦建立查看流水，就把"本人同意"从一次性认领变成持续义务，合规口径要重写 | 二者先到者触发：① #17 第一期落地（招聘方成为可引用主体，"谁"才答得出）② 法务对 R2 的意见要求"查看告知"，或出现候选人投诉"莫名被企业面试" | [design-recruiter-roles-20260925.md](design-recruiter-roles-20260925.md) §2.3 / §6.2、[待拍板决策清单 #1-A](待拍板决策清单-20260910.md)、PRD R2/R8 |
