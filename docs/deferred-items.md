...
| #12 Gitee / 多证据源接入 | 契约层已预留 `EvidenceSource` 抽象；先用 GitHub 跑通 L0/L1 内核，避免最早期两套 API/合规翻倍 | **GitHub 内核 L0/L1 验证通过后即并行（M1 末/M2 初）**——决策 #4 已改"海内外同步"，Gitee 自 P3 提前为第一个并行证据源 | [待拍板决策清单 #12/#4](待拍板决策清单-20260910.md) |
