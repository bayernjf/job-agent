...
> 文档版本：v0.1（草稿）
> 日期：2026-09-10
> 状态：**草稿，含 4 项待拍板决策（见第 12 章）**；未决处均以 `【待决策 #n】` 标注，未把建议当作既定结论
> 关联文档：
...
- **非目标（本期明确不追求）**：不做双边交易闭环、不追求岗位 / 简历规模、不做支付。
...
| 海外 / 国内用户 | 先海外 GitHub 生态【待决策 #4】 | 国内后续兼容 Gitee 与本地化合规 |
...
- **US-3（招聘方，轻量）**：作为招聘方，我粘贴一个公开账号，看到候选人的能力标签、真实性分级与"基于其真实项目"的面试问题，用于准备面试。【是否进入 P0 取决于待决策 #1】
...
- 开发者 GitHub OAuth **本人认领**与完整报告、改进建议；【与待决策 #1 相关】
- 招聘方"公开轻预览"界面与面试问题增强；
...
- **P3**：L2 clone 静态分析（隔离沙箱）、L4 跨库 / 身份聚合、Gitee 接入、非技术岗多元证据源。
...
| **真实性分级（Authenticity）** | `likely_authentic / mixed_signals / suspicious / insufficient_data` 四级 + 置信度 + 可解释信号，**不使用单一百分比分数**【待决策 #2 确认】 |
...
- **验收标准**：
  ...
  4. 落地页文案中删除 / 替换"真实度 98%"这类与正式口径冲突的示意（示意数据需明确标注）。【与待决策 #2 联动】

### F8 本人 OAuth 认领（P1）

- **描述**：开发者 OAuth 授权后认领本人公开画像，获得完整报告与改进建议。【范围受待决策 #1 约束】
...
| NFR-8 | 可访问 / 国际化 | 落地页与报告先中文（兼顾英文术语）；为后续中英文与海外访问预留结构 |
...
- **自动化决策与画像风险**：对"可识别个人"输出能力 / 真实性结论并可能用于雇佣，方向上涉及画像与自动化决策相关规则（GDPR 相关条款、中国《个人信息保护法》）。**具体法条与合规动作待查原文 / 法务（R2）**，设计上先做到：
...
| 4 | 是否先押海外 GitHub 生态、Gitee 后置？ | 先海外，Gitee 后置 | 市场、架构多平台优先级 |

---

## 13. 里程碑（建议，待决策后定档）

1. **M1 分析内核 + 报告（P0）**：L0/L1 管道、AbilityProfile v0.1、报告页、落地页 demo 接真；并行跑标注样本实验。
2. **M2 认领与轻 B 端（P1）**：OAuth 认领、改进建议、公开轻预览、异议处理、L3 语义问题增强。
3. **M3 双边闭环（P2）**：岗位、匹配、投递、结果标签回流、付费验证。
4. **M4 多平台 / 非技术（P3）**：L2 沙箱静态分析、L4 聚合、Gitee、多元证据源。
...
| R7 | 国内 GitHub 访问不稳定 | 中 | 先海外市场；后续 Gitee / 部署优化 | 后置 |
