...
- **内核散文的语言归属（与上条并列，改画像输出或交付物渲染前必读）**：`analyzer-core` 写进快照的句子（`summary.headline`、`collaboration.prSummary`、`activity.cadenceSummary`、信号 `label`/`detail`、`improvementSuggestions`）是**数据层英文原文**，全链路存档与 API 都以此为准；面向读者的句子由 **`packages/shared` 的 composer** 现拼——`composeHeadline` / `composePrSummary` / `composeSeniorityBand` / `composeSkillDepthLabel` / `composeImprovementSuggestion`，一律是 `(facts | code, locale) => string`。三条硬规矩：①渲染侧**只认 `code` 与 facts，绝不按英文句子匹配**（改文案不能让渲染分支失效）；②**取不到的事实不许编**，退回快照英文原句也比猜数好（例：外部 merged PR 数不在快照里，`externalMergedContributions` 是去重仓库名 ≤5，不能当 PR 数用）；③若内核改调 composer 而**输出必须逐字节不变，用写在测试里的旧字面量钉住**，这样不升 `RULE_VERSION`（升了就意味着同一版本对应两套输出，缓存与分享链接失去可复现性）。内核散文要新增可本地化字段时，先补 `facts` 再升规则版本（见 T33）。
