...
- 阶段：**M1 核心开发 + 工程化补全完成，P1·Chrome 扩展真实环境冒烟通过，进入扩展稳定期**。M1·W1~W4、W3-6 SQLite/Postgres 双方言、#8 去风险、端到端联调、真实性三轮校准均已完成。**2026-09-12 一次性完成 10 项工程化补全**（见活跃待办 item 11）：Playwright E2E 主链路（顺带修复两个 React island 缺 `client:load` 致浏览器内无交互的真实 bug）、Dockerfile/docker-compose、embedded-postgres 真实 PG 行为测试、pre-commit 卡死修复、画像缓存、CLI markdown/html 导出、docs/API.md、report i18n 测试补齐增强、waitlist CLI 只读子命令（`4474978`）。**真实性第三轮双向校准完成**：标注账号扩到 26 个（22 正 + 4 负），真实账号 0 误报为 suspicious、可疑账号 0 漏报为 likely_authentic，analyzer-core 32 测试。**P1·Chrome 扩展第 3 步真实环境冒烟完成（2026-09-12）**：真实 Greenhouse 岗位页跑通「按钮→面板→画像→一键填充」四环，first_name 真实写入；发现并修复 4 个真实问题（exportable 端点/双 root/SPA 保活/id 字段匹配），4 个原子提交未 push。全仓 typecheck/test/build 三件套全绿。**下一步：真实用户装扩展试用 + 分发策略拍板（Chrome Web Store vs CRX）与正式图标；Lever/Workday 适配器冒烟；summary→question_* 映射**。
...
9. **P1·Chrome 扩展一键填充**（决策 #15，2026-09-11 拍板）：支持 Workday/Greenhouse/Lever 三大 ATS，填充数据来自可信画像；只做用户主动触发的一键填充，不做全自动后台投递。前置：packages/shared 预留可导出画像数据结构（已落地）。
   ...
   - **下一步（P1 扩展）**：真实用户装扩展试用（本地 api+worker 已可跑）；分发策略拍板（Chrome Web Store vs CRX 手动安装）与正式图标；Lever/Workday 适配器真实页面冒烟；summary 到 Greenhouse question_* 自定义字段映射（当前 cover_letter 为 file input、summary 字段不命中）。
...
- **P1 扩展开放项**：分发策略未拍板（Chrome Web Store vs CRX 手动安装）、正式图标未做；Lever/Workday 适配器仅单测覆盖、未真实页面冒烟；Greenhouse 表单的 summary 需映射到 question_* 自定义字段（当前 cover_letter 为 file input 不命中，未填）；填充字段命中依赖用户本地补填（email/phone/linkedin/location，仅存本机 localStorage）。
...
- **本地领先 `origin/dev` 6 个提交未 push**（dev 默认不 push，需用户明确要求）：`ed69003`（api exportable 端点）、`d959c83`（扩展读 exportable）、`54b513e`（扩展 overlay 保活）、`3e74f40`（扩展字段 id/iframe 匹配）为本轮新增，另含 `975e367`（扩展骨架）、`4474978`（waitlist CLI）。工程化补全批次（缓存/CLI 导出/E2E/Docker/API 文档等）与 waitlist CLI 均已按原子规则提交。
- 工作区当前仅 handoff.md、docs/API.md 两处文档更新未提交。
